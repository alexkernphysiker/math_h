...
TEST(RandomValueGenerator,Throwing){
	...
	EXPECT_THROW(RandomValueGenerator<double>(f,0,1,0),Error<LinearInterpolation_fixedsize<double>>);
	EXPECT_THROW(RandomValueGenerator<double>(f,0,1,-1),Error<LinearInterpolation_fixedsize<double>>);
	EXPECT_THROW(RandomValueGenerator<double>(f,0,1,1),Error<LinearInterpolation_fixedsize<double>>);
	EXPECT_THROW(RandomValueGenerator<double>(f,0,-1,2),Error<LinearInterpolation_fixedsize<double>>);
	EXPECT_THROW(RandomValueGenerator<double>(f,0,0,2),Error<LinearInterpolation_fixedsize<double>>);
	EXPECT_NO_THROW(RandomValueGenerator<double>(f,0,1,2));
	EXPECT_THROW(RandomValueGenerator<double>(z,0,1,2),Error<RandomValueGenerator<double>>);
	EXPECT_THROW(RandomValueGenerator<double>(z,0,0,0),Error<LinearInterpolation_fixedsize<double>>);
	auto n=[](double x){return sin(10*x);};
	EXPECT_THROW(RandomValueGenerator<double>(n,0,1,100),Error<RandomValueGenerator<double>>);
}

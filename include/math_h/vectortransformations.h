...
public:
    ...
    inline Direction(const Direction<Dimensions, numt2> &source)
        : sign(source.___sign()) {}
    inline Direction(const VType &v)
        : sign(v.x() >= 0) {}

    template<class... Args>
    inline Direction(const std::tuple<Args...> &args)
        : sign(std::get<0>(args)) {}
    ...
    inline auto Rotations() const
    {
        return sign ? rows(vec(numt(1))) : rows(vec(numt(-1)));
    }

    inline auto AntiRotations() const
    {
        return sign ? rows(vec(numt(1))) : rows(vec(numt(-1)));
    }
};
...
public:
    ...
    inline const auto &phi()const
    {
        return m_phi;
    }

    Direction(const VType &V)
        : m_phi(atan2(V.y(), V.x())) {}
...
public:
    ...
    Direction(const VType &V)
        : m_ld(V.___minus_one_component()), m_theta(acos(V.template component<Dimensions>() / V.length())) {}
    ...
    auto Rotations()const
    {
        return Matrix<Dimensions, VType>(
    ...
    auto AntiRotations()const
    {
        return Matrix<Dimensions, VType>::template RotationInPlane<3, 1>(-m_theta)
...
public:
    ...
    template<class numt2>
    inline Direction(const Direction<Dimensions, numt2> &source)
        : m_ld(source.___minus_one_component()), m_theta(source.___last_component())
    {
        ___theta_check();
    }

    template<class... Args>
    inline Direction(const std::tuple<Args...> &v)
        : m_ld(v), m_theta(std::get<Thetas>(v))
    {
        ___theta_check();
    }
    ...
    auto AntiRotations()const
    {
        return Matrix<Dimensions, VType>::template RotationInPlane < Dimensions, Dimensions - 1 > (-m_theta) 
            * Matrix<Dimensions, VType>(
                m_ld.AntiRotations().AddColumn(DirectionN::VType::zero()),
                VType::template basis_vector<Dimensions>()
            );
...
template<class numt = double>
inline auto Rotation(const numt &theta)
{
...
template<class numt = double>
inline auto Rotation(const Direction<3, numt> &axis, const numt &theta)
{
...
namespace details{
    ...
    template<class numt>
    inline auto SkewM(const Vector<2, numt> &A)
    {
        return rows(vec(-AC(2), AC(1)));
    }

    template<class numt>
    inline auto SkewM(const Vector<3, numt> &A)
    {
        return rows(
            vec(ZeRo, -AC(3), AC(2)),
            vec(AC(3),  ZeRo, -AC(1)),
    ...
    template<class numt>
    inline auto SkewM(const Vector<7, numt> &A)
    {
        return rows(

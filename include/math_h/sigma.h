...
public:
	...
	numt getAverage()const{
		int sz=m_list.size();
		if(sz<=0)
			throw Error<Sigma>("No data to check. For average needed at least one element.");
		return m_sum/sz;
	}
	...
	numt getSigmaSqr()const{
		int sz=m_list.size();
		if(sz<=1)
			throw Error<Sigma>("No data to check. for sigma needed at least two elements.");
...
public:
	...
	WeightedAverageCalculator &AddValue(numt X,numt deltaX){
		if(deltaX<=0)
			throw Error<WeightedAverageCalculator>("Attempt to add point with zero error.");
	...
	numt Average()const{
		if(Wnorm<=0)throw Error<WeightedAverageCalculator>("Attempt to check empty data");
		return Sum/Wnorm;
	}
	numt Sigma()const{
		if(Wnorm<=0)throw Error<WeightedAverageCalculator>("Attempt to check empty data");

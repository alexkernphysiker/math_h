...
namespace MathTemplates{
	...
	public:
		...
		const numt NumCompare(const numt&x){return pow(Value-x,2)/pow(Error,2);}
		const numt NumCompare(const value&x){return pow(Value-x.Value,2)/(pow(Error,2)+pow(x.Error,2));}
